...
Here we explore the interplay between some of these ``key factors'' of fitness in a simplified, territorial model of growth, dispersal and competition, with the broader aim of adding some ecological realism to the time-depedendent, genetically-focused, population genetic models of evolution. To achieve this, we develop a generalized version of the classic lottery model of \cite{chesson_1981}, which has two features that make it well suited for this role. 

First, the lottery representation of competition is much simpler than it is in other commonly used ecological models, such as coefficients for the pairwise effects of types on each other (e.g. the $\alpha$ coefficients in the generalized Lotka-Volterra equations), or  explicit resource consumption \citep{tilman_1982}. Mature individuals (``adults'') each require their own territory, whereas newborn individuals (``propagules'') disperse to, and subsequently compete for, territories made available by the death of adults. Territorial contest among propagules leaves a single victorious adult per territory, the victor chosen at random from the propagules present, with probabilities weighted by a coefficient for each type representing competitive ability, akin to a lottery \citep{sale_77}. Second, as we will see, the lottery model is closely connected to one of the central models of population genetics, the Wright-Fisher model of genetic drift. 

However, the classic lottery model breaks down at low densities (few propagules dispersing to each territory; see ``Model''). This was not a limitation in its original application to reef fishes, where a huge number of larvae from each species compete to secure territories \citep{chesson_1981}, but is a critical limitation for studying density-dependent selection. We analytically extend the classic lottery model to correctly account for low density behavior.

We broadly follow the spirit of MacArthur's $r$/$K$ selection scheme in that our model is intended to account for fundamentally different forms of selection without getting  entangled in the intricacies of particular ecological scenarios. 

However, rather than building directly on MacArthur's formalism and its later extensions using Lotka-Volterra equations to incorporate competition (``$\alpha$-selection'') \citep{gill_1974,case_1974,joshi_2001},

 our model is devised more with Grime's C/S/R scheme in mind, and represents a quantitative formalization of how C/S/R manifests at the level of within-population genotypic evolution (as opposed to phenotypic divergence between species). 
 
 This choice is motivated in part by the substantial empirical support for C/S/R-like schemes, and in part by the failings of the $r$/$K$ low/high density dichotomy --- many growth ability traits will confer advantages at both low and high densities (more details in the Discussion). 



Building on the r/K and C/S/R schemes, a central question is how fitness depends on population density, intrinsic birth/death rates and competitive ability.  
 ...
 However, while trait classification schemes are  grounded in trait data, they are verbal and descriptive rather than mathematical, a recognized hindrance to their broader applicability (e.g. \citealt{tilman_2007}).
