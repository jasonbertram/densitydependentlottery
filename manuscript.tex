...
The validity of the approximations in Eq.~\eqref{eq:meanfieldr} and \eqref{eq:meanfielda} 

If this standard deviation is small as claimed, then the mean value gives an accurate representation of the values taken by $\sum_{j\neq i} c_j x_j$ across territories.


are relies on the standard deviation $\sigma_{\tilde{q}}(\sum_{j\neq i} c_j x_j)$ (with respect to $\tilde{q}$) being much smaller than the corresponding mean $\langle\sum_{j\neq i} c_j x_j\rangle_{\tilde{p}}$.  
