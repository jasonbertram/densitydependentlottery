...
We now consider periodically varying birth and death rates, as could occur due to seasonal cycles. For example, natural \textit{Drosophila} populations expand rapidly in the warmer months when fruit is abundant,  and are carried through the colder months by a much smaller number of dormant individuals. Population density undergoes seasonal cycles, and within these cycles, hundreds of polymorphisms also cycle in frequency, some of which may potentially have been doing so for millions of years \citep{bergland_14}.

Figure \ref{fluctuatingselection} shows the behavior of Eq. \eqref{eq:master} for an example where $b$ and $d$ cycle between zero and positive values (``summers'' with rapid growth and no mortality, and ``winters'' with positive mortality and no growth). 


Selection alternatively favors the $b$ specialist 


. However, due to the storage effect neither has an advantage over a full environemntal cycle, and
