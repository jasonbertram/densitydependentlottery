...
Here we describe how Grime's ``disturbed'', ``stressful'' and ``ideal'' environments can be captured mathematically in our scheme. To proceed, we  map these verbally-defined environments to quantitative parameter regimes in our model. 
...
\end{tabular}
\caption{\label{fig:table} The mathematical realization of Grime's three environmental extremes in our model, as well as the high-density variant of the stressful environment. *Mortality affects both adults and juveniles in the disturbed environment, with $\Delta_+ n_i$ replaced by $(1-d_i)\Delta_+ n_i$.}
\end{figure}

Given our mathematical representation of Grime's environmental extremes, summarized in Fig.~\ref{fig:table}, the expected direction of evolution is determined by three factors. First, the probability that an adaptive mutant survives its initial low abundance phase and begins to grow deterministically is proportional to its expected absolute growth rate \citep{haldane_1927,uecker_2011}. By comparison, neutral mutations rarely fix (probability of order $1/N$). Thus $b$, $c$ and $d$ will tend to change in the direction of greatest fitness increase. Under Eq. \eqref{eq:master}, proportional changes of the form $b_j=(1+\epsilon) b_i$ or $d_j=(1+\epsilon) d_i$ have identical fitness effects

When propagule density is low $L\ll 1$, $c$ does not contribute to fitness. 

Second, the fact that some mutations are more common than others can change the direction of evolution \citep{yampolsky_2001}, and in particular, very few mutations can perform difficult trait changes such as increasing $b$ under stress. These mutational constraints are different under different environments (Fig.~\ref{fig:table}). 

Finally, there may be pleiotropy between $b$, $c$, and $d$, which could be due to trade-offs between them. Here we do not attempt to account for the complications posed by pleiotropy, and treat $b$, $c$ and $d$ independently. 
...
Now suppose that $j$ is better at securing territories ($c_j>c_i$), that $i$ is better at producing propagules ($b_i>b_j$), with $d_i=d_j$. Coexistence occurs if $j$ will invade an $i$-dominated population, but $i$ will also invade a $j$-dominated population (``mutual invasion''). If $b_i$ is so large that $L\gg 1$ when $i$ is dominant, and $b_j$ is so small that $L\ll 1$ when $j$ is dominant, then, combining Eqs. \eqref{eq:equil} and \eqref{eq:invad}, we find that $i$ invades $j$ because $b_i>b_j$, while $j$ invades $i$ provided that
\begin{equation}
b_jc_jR_j-b_i c_i A_i>0. \label{eq:jinvadcoex}
\end{equation}
Thus, coexistence occurs if $c_j/c_i$ is large enough. This is a version of the classic competition-colonization trade-off \citep{tilman_94,levins_71}: the competitor ($c$-specialist) leaves many territories unoccupied (low $L$) due to its poor colonization ability (low $b$), which the colonizer ($b$-specialist) can then exploit. A similar argument applies for coexistence between high-$c$ and low-$d$ specialists; a ``competition-longevity'' trade-off \citep{tilman_94}. These forms of co-existence require density dependence, and are not present in the classic lottery model. Mutual invasion is not possible between $b$- and $d$-specialists. 
...
In this Appendix we consider the invasion of mutants into a population with a single resident type $i$ in equilibrium under the different environments from section ``Primary strategies and Grime's triangle''. We first evaluate the ideal and stressful environments (the treatment of mortality is different for disturbed environments). 

Summing Eq. \eqref{eq:growthsumuncoupled} over genotypes, the change in $N$ is given by  
\begin{equation}
\Delta N=U(1-e^{-L})-\sum_i d_i n_i,\label{eq:deltN}
\end{equation}
where $U=\sum_i b_i n_i/L$. In equilibrium ($\Delta N=0$), this implies $L/(1-e^{-L})=b_i/d_i $. The fitness of a low frequency invader $j$ is
\begin{equation}
\Delta n_j/n_j \approx b_j \left(e^{-L}+R_jc_j\right)-d_j \label{eq:invad}
\end{equation}
where $R_j\approx (1-e^{-L})/\left(c_j+\frac{L-1-e^{-L}}{1-(1+L)e^{-L}}\right)$, and we set the reference competitiveness at $c_i=1$ without loss of generality.

According our representation of Grime's stressful environment, $b\ll 1$, $d\approx b$ and thus $L\ll 1$. Then Eq. \eqref{eq:invad} becomes $\Delta n_j/n_j \approx b_j-d_j$. Mutations which improve $b$ or $d$ by the same fraction $\epsilon$ (such that $b_j = (1+ \epsilon)b_i$ or $d_j = (1- \epsilon)d_i$) will therefore yield identical fitness benefits. Stressful environments will therefore favor $d$-specialist ``tolerators'', which bring their mortality rates down enough to enable survival in the face of low $b$, only if stress makes it harder to increase $b$ than lower $d$, such that adaptive mutations of a given effect size $\epsilon$ occur more often for $d$. 

In Grime's ideal environment, $d$ is much smaller than $1$ whereas $b$ is not. Thus, $1 \ll b_i/d_i = L/(1-e^{-L}) \approx L$, and Eq. \eqref{eq:invad} becomes 
\begin{equation}
\Delta n_j/n_j \approx \frac{b_j c_j}{c_j + L-1}-d_j\approx \frac{b_j c_j}{L}-d_j, \label{eq:highLinvad}
\end{equation}
where the last approximation follows from the fact that $c_j$ will be of similar magnitude to $c_i=1$ (ignoring mutations with very large effect sizes; compare with Eq. \eqref{eq:lottery}, where the same expression comes from assuming that $L$ is effectively infinite). It is easily verified from Eq. \eqref{eq:highLinvad} that improving $b$, $c$, or $d$ by a fraction $\epsilon$ yields identical fitness benefits. This may seem counter-intuitive in that reducing an already tiny mortality rate by a fraction $\epsilon$ seems to yield marginal benefits compared to increasing a  potentially large propagule production $b$ by the same fraction. However, the relative magnitudes of $b$ and $d$ are irrelevant, because the ancestral resident type's density-regulated birth rate $b_i c_i/L$ exactly balances its mortality $d_i$ in equilibrium.

Biases in the direction of evolution of $b$, $c$ or $d$ must again follow from biases in mutation availability or trade-offs/pleiotropy. The same argument applies for high-$L$ interpretation of the stress regime. 

To represent Grime's disturbed environment, we account for juvenile mortality due to disturbance by replacing $\Delta_+ n_i$ with $(1-d_i)\Delta_+ n_i$. For a single type $i$ in equilibrium, Eq. \eqref{eq:deltN} becomes 
\begin{equation}
\Delta N=(1-d_i)U(1-e^{-L})-d_i N,\label{eq:deltNdist}
\end{equation}
which reaches equilibrium when $L/(1-e^{-L})=(1-d_i)b_i/d_i$ (note that population persistence now requires $(1-d_i)b_i>d_i$). We represent Grime's disturbed environment by $d_i\approx 1$ and $(1-d_i)b_i\approx d_i$, so that $L\ll 1$. A mutant's invasion fitness is then given by
\begin{equation}
\Delta n_j/n_j = (1-d_j)b_j - d_j.
\end{equation}
The effects of improving $b$ and $d$ are now no longer identical. Increasing $b$ by a fraction $\epsilon$ gives $\Delta n_j/n_j = \epsilon (1-d_i)b_i$, whereas reducing $d$ by the same fraction gives $\Delta n_j/n_j = \epsilon b_i$, a much greater advantage given that $d_i\approx 1$. Thus, selection intrinsically favors reducing $d$ in disturbed environments. 
