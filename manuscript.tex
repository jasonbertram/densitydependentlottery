...
Suppose that we simply used the conditional distributions $p({\mathbf x}|x_i=1,X_i\geq 1)$ and $p({\mathbf x}|x_i\geq 2)$ to calculate the effective means, such that $\langle \rangle_r=\langle \rangle_{p({\mathbf x}|x_i=1,X_i\geq 1)}$ and $\langle \rangle_a=\langle \rangle_{p({\mathbf x}|x_i\geq 2)}$. Then, again using the example from the previous paragraph ($l_1\ll 1$, $l_2\gg 1$), the right hand side of Eq.~\eqref{eq:meanfieldr} would be $\approx 1/(l_2+1)$, and so $\Delta_r n_1/m_1 \approx 1/(l_2+1)$ in Eq.~\eqref{eq:deltr}. Similarly, $\sum_j \langle x_j\rangle_a = l_2$ in Eq.~\eqref{eq:meanfielda}, and so $\Delta_a n_2/m_2 \approx 1/l_2$. Thus, the rare type is pathologically predicted to decline in frequency under this approximation. This inconsistency occurs because the expected total numbers of propagules in the respective groups of territories are different; $\langle X \rangle_{p({\mathbf x}|x_1=1,X_1\geq 1)}\approx l_2+1 > \langle X \rangle_{p({\mathbf x}|x_2\geq 2)}\approx l_2 $. By approximating the expected outcome of lottery competition with effective means, the rare type is effectively assumed to experience more intense lottery competition. 

The effective distributions $\hat{p}$ are devised to avoid this pathology by forcing $\langle X \rangle$ to be same in all territories. They are defined as follows. For each propagule number $x_i$, $\hat{p}(x_i)=p$ which is independent of the other number

 by making the approximation that the distribution for $X$ is the same in all territories. 

 Then, in territories where one focal propagule and at least one non-focal propagule are present, the effective distribution is defined by 
\begin{equation}
