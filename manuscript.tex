...
The expected direction of evolution is determined by three factors. First, there may be pleiotropy between $b$, $c$, and $d$, which could be due to trade-offs between them. Second, adaptive mutations in one trait may occur more frequently than in others, which can change the direction of evolution \citep{yampolsky_2001}. Quantifying how traits are affected by mutations is beyond the scope of this article, and so we will not evaluate either of these effects: mutations affect $b$, $c$ and $d$ independently, and they all have the same distribution of beneficial fitness effects.

The third effect is that selection can favor some trait changes over others. The probability that an adaptive mutant survives its initial low abundance phase and begins to grow deterministically is proportional to its expected absolute growth rate \citep{haldane_1927,uecker_2011}. Very few neutral mutations fix (probability of order $1/N$). Thus $b$, $c$ and $d$ will tend to change in the direction of greatest fitness increase. 
