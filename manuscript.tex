...
In this Appendix we consider the invasion of mutants into a population with a single type $i$ in equilibrium under the different environments from section ``Primary strategies and Grime's triangle''. We first evaluate the ideal and stressful environments (the treatment of mortality is different for disturbed environments). 

Summing Eq. \eqref{eq:master} over genotypes, the change in $N$ is given by  
\begin{equation}
...
According our representation of Grime's stressful environment, $b\ll 1$, $d\approx b$ and thus $L\ll 1$. Then Eq. \eqref{eq:invad} becomes $\Delta n_j/n_j \approx b_j-d_j$. 

According to our definition of an ideal environment, $d$ is much smaller than $1$ whereas $b$ is not. Thus, $1 \ll b_i/d_i = L/(1-e^{-L}) \approx L$, and Eq. \eqref{eq:invad} becomes 
\begin{equation}
\Delta n_j/n_j \approx \frac{b_j c_j}{c_j + L-1}-d_j, 
\end{equation}
