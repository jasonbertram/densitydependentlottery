...
How does Fig. 4 compare to Grime's C/S/R strategies? In disturbed environments, the ruderal (R) strategy is characterized by high fecundity, wide dispersal and short generation times. 

We predict evolution for higher $b$, but not higher $c$ or lower $d$ (constrained to be large), amounts to focusing on fecundity. While there is no dispersal limitation in our model, propagules must be minimally effective at dispersal since our model only includes propagules which will enter territorial contests on unoccupied sites; 

 Shorter generation times are thought to be a mechanism for increasing the chance that the reproductive cycle completes before disturbance-induced death

The evolution of the generation time itself is beyond the scope of our model, but it could
