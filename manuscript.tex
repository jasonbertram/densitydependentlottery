...
In ideal environments, we predict evolution for higher $b$ and $c$, but not lower $d$ (which is already small). In plants and corals, the ability to secure territories $c$ is strongly associated with rapid individual growth, and rapid individual growth is a defining feature of the competitor trait cluster \cite{grime_1977,darling_2012}. Evolution for higher $b$ (which is closely related to the intrinsic low-density growth rate $r=b-d$ ) is not usually thought to be the expected outcome in high-density competitive environments. We will discuss this further below in the context of MacArthur's $r$/$K$ scheme. For now we simply note that the analogue of Grime's ideal environment for fishes occurs when resources limitations are not severe and the environment varies predictably over large temporal and spatial scales \cite{winemiller_1992,winemiller_2015}. The associated ``periodic'' strategy is characterized by enormous brood sizes, suggesting a strategy of ensuring that more propagules actually end up contesting areas favorable for development (higher $b$). 

Indeed, the classic lottery model was originally applied to territorial reef fishes, and consequently the $l_i\gg 1$ assumption was 
