...
In the classic lottery model \citep{chesson_1981}, unoccupied territories are saturated with propagules from every genotype i.e. the mean territorial propagule density $l_i=m_i/U$ is much larger than one for all genotypes. The law of large numbers then implies that the composition of propagules in each territory will not deviate appreciably from the mean composition $l_1,l_2,\ldots,l_G$ ($G$ is the number of genotypes present), and so the probability that genotype $i$ wins any particular unoccupied territory is approximately $c_i l_i/\sum_j c_j l_j$. Since $T$ is large (by assumption), genotype $i$ wins approximately $c_i l_i/\sum_j c_j l_j$ of the $U$ available territories, and so the increase in $n_i$ due to territorial acquisition in the classic lottery model is given by
\begin{equation}
\Delta_+ n_i(t)=U(t)\frac{c_i l_i}{\sum_j c_j l_j}= b_i n_i\frac{1}{L}\frac{c_i}{\overline{c}}, \label{eq:lottery}
\end{equation}
...
Unlike the classic lottery model, we do not restrict ourselves to high propagule densities. In our extension to arbitrary densities, we expect that $p_1(x_1)\ldots p_G(x_G)$ of the $U$ unoccupied territories will have the propagule composition $x_1,\ldots,x_G$. Within these $U(t) p_1(x_1)\ldots p_G(x_G)$ territories, 


Since $T$ and the $n_i$ are large enough that we can ignore fluctuations in this partitioning of the unoccupied territories as well as the number of victories, which implies
\begin{equation}
...
%Consequently, we can replace $x_i$ with $l_i$ in Eq. \eqref{eq:growthsumuncoupled}.
