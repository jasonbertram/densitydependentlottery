...
\end{tabular}
\caption{\label{fig:table} The mathematical realization of Grime's three environmental extremes in our model, as well as the high-density (HD) variant of the stressful environment. *Mortality affects both adults and juveniles in the disturbed environment, with $\Delta_+ n_i$ replaced by $(1-d_i)\Delta_+ n_i$.}
\end{figure}

The expected direction of evolution is determined by three factors. First, there may be pleiotropy between $b$, $c$, and $d$, which could be due to trade-offs between them. Here we do not attempt to account for the complications posed by pleiotropy, and treat $b$, $c$ and $d$ independently. 

Second, some mutations are more common than others, which can change the direction of evolution \citep{yampolsky_2001}. In particular, very few mutations can perform difficult trait changes such as increasing resilience to intense physical disturbance. Quantifying which trait changes are more mutationally accessible than others is beyond the scope of this article, and so we will assume that $b$, $c$ and $d$ all have the same distribution of beneficial fitness effects.

Finally, the probability that an adaptive mutant survives its initial low abundance phase and begins to grow deterministically is proportional to its expected absolute growth rate \citep{haldane_1927,uecker_2011}. Very few neutral mutations fix (probability of order $1/N$). Thus $b$, $c$ and $d$ will tend to change in the direction of greatest fitness increase. 

Under Eq. \eqref{eq:master}, proportional $b$- and $d$-mutations of the form $b_j=(1+\epsilon) b_i$ and $d_j=(1-\epsilon) d_i$ have identical fitness effects (here $j$ is the mutant type; Appendix C). When propagule density is high  $L\gg 1$, the same is true for $c$-mutations of the form $c_j=(1+\epsilon) c_i$. When propagule density is low $L\ll 1$, $c$ does not contribute to fitness and $c$-mutations are neutral. Thus, in ideal and stressed environments, selection under Eq. \eqref{eq:master} does not intrinsically favor one trait improvement over another, apart from the irrelevance of $c$ at low density. 

The situation in disturbed environments is more complicated. The bulk of the high mortality $d\approx 1$ is environmental and unavoidable. Thus, mutational changes in $d$ are intrinsically less effective than mutation change in $b$. However, this effect is easily swamped by the fact that disturbance mortality affects both juvenile recruitment and adult mortality.
