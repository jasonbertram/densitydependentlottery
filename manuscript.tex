...
Note that the competitive ability coefficients $c_i$ represent a strictly relative aspect of fitness in the sense that they only influence population size $N$ indirectly by changing genotype frequencies, which may in turn change the population mean birth and death rates. This can be seen by summing Eq. \eqref{eq:delttot} over genotypes to get the  change in population size $N$, 
\begin{equation}
\Delta N=U(1-e^{-L})-\sum_i d_i n_i,
\end{equation}
where $L=\sum_j l_j$ is the overall propagule density; $\Delta N$ is independent of the $c_i$.
...
We now discuss which mutational changes in the traits $b, c$ and $d$ will be favored under different environmental conditions. Of particular interest are Grime's ``disturbance'', ``stress'' and ``ideal'' environmental archetypes. To proceed, we map these archetypes to different parameter regimes in our model. 

The disturbance archetype is characterized by unavoidably high extrinsic mortality caused by the direct physical destruction of individuals, to the extent that most territories are not used most of the time ($N/K\ll 1$).


, and reproduction is a race against time 





This means that, even though $b_i$ may be quite high, it cannot greatly exceed $d_i$, and that  propagule densities are low $l_i\ll 1$. The terms proportional to $c_i/\overline{c}$ in Eq. \eqref{eq:master} are then negligible, and $\Delta_+ n_i$ depends primarily on $b_i$.

The ideal environmental archetype is characterized by the absence of stress and disturbance, low extrinsic mortality and high [BLAH] such that $b_i$ significantly exceeds $d_i$, and so $N/K$ is high, and $L\gg 1$. In this case, territorial contests are important.
