...
\caption{\label{fig:simcomp} Comparison of mean field approximation Eq. \eqref{eq:master} with simulations. Per-propagule success probability $\Delta_+ n_i/l_i U$ from Eq. \eqref{eq:master} and its three components, the classic lottery model, and  individual-based simulations of random dispersal and lottery competition. Two types are present, a rare type with $c_1=1.5$, and a common type with $c_2=1$. Simulation points are almost invisible in for the common type due to near exact agreement with Eq. \eqref{eq:master}. Dashed lines in show the breakdown of the classic lottery model. Parameters: $m_1=10^4$ and $m_2=9\times 10^4$ and $U$ is varied between $5\times 10^3$ and $10^6$.} 
\end{figure}
