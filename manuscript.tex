...
&=\frac{1}{1-(1+L)e^{-L}}\sum_{X=2}^{\infty} P(k) p({\mathbf x_i}|X_i=X-1),
\end{align}
and so
\begin{align}
...
\end{align}
The inner sum over ${\mathbf x_i}$ is the mean number of $j$ propagules that will be found in a territory which received $X-1$ nonfocal propagules, which is equal to $\frac{l_j}{L-l_j}(X-1)$. Thus, 
\begin{align}
...
For analyzing the relative fluctuations in $\sum_{j\neq i} c_j x_j$, Eq. \eqref{eq:meanxjrare} is unnecessarily complicated. We instead use the following approximation. Rather than evaluating the situation in each territory after dispersal as above, we replace $\tilde{p}$ by $\tilde{q}$, defined as the ${\mathbf x_i}$ Poisson dispersal probabilities conditional on $X_i>1$, independently of the outcome of $x_i$. This gives $\langle x_j \rangle_{\tilde{q}}=l_j/C$, 
\begin{equation}
...
\end{equation}
and using the fact that $\sigma_{\tilde{q}}(x_j,x_k)$ and the first term in Eq. \eqref{eq:varr} are negative because $C<1$, we obtain an upper bound on the the relative fluctuations in $\sum_{j\neq i} c_j x_j$, 
\begin{equation}
...
&=\sum_{X}P(X|x_i\geq 2)\sum_{x_i}p(x_i|x_i\geq 2,X) \frac{l_j(X-x_i)}{L-l_j} \nonumber\\
&=\frac{l_j}{L-l_j}\left[\sum_{X}P(X|x_i\geq 2)X - \sum_{x_i}p(x_i|x_i\geq 2) x_i \right]\nonumber\\
&=\frac{l_j}{L-l_j}\left( L\frac{1-e^{-L}}{1-(1+L)e^{-L}}- l_i\frac{1-e^{-l_i}}{1-(1+l_i)e^{-l_i}}\right) 
\end{align}
The mean focal genotype abundance is, 
\begin{align}
