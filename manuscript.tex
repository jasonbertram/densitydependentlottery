...
In a huge class of evolutionary models, and in many introductory texts, selection is described in terms of shifts in genotypic frequencies (e.g. \cite[pp. 468]{barton_2007}). Genotypes are assigned constant selection coefficients describing how their relative proportions change over time. These selective shifts in frequency result from two fundamentally different population processes: i) in the absence of interactions between individuals, genotypic growth rates differ; ii) as a result of competitive interactions between individuals, some genotypes displace others. Population density is the key factor that controls whether individuals interact, thus controlling the relative contributions of these forms of selection.

The distinction between these forms of selection was perceived long ago by Kostitzin \citep{kostitzin_1939,christiansen_2004}, but is mostly associated with MacArthur's $r$/$K$ scheme \citep{macarthur_1962,macarthur_1967} and the following literature on density-dependent selection. 
...
Although $r$ and $K$ are named after the common ``carrying capacity'' parameterization of the logistic model, the concepts of $r$- and $K$-selection are not restricted to that model \citep{macarthur_1967}.
 ...
 This immediately poses a challenge: any description of evolution grounded in abundances needs to specify how population density is regulated, as well as how fitness depends on density, which may differ between genotypes (density-dependent selection). As a result, 
