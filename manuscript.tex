...
A variety of different measures of fitness are used to quantify the effects of selection. Some widely used examples in evolutionary ecology are expected lifetime reproductive ratio $R_0$,  intrinsic growth rate $r$, saturation population density (often labeled ``$K$'') \citep{benton_2000}, and invasion fitness \citep{metz_1992}. In addition, ``relative fitness'' is the standard in much of evolutionary biology, particularly evolutionary genetics, where attention is generally restricted to relative genotypic proportions \cite[pp. 468]{barton_2007}. 

In principle these fitness measures should all be grounded in population ecology, because ultimately selection operates through the environmentally-dependent differential reproduction and survival of individuals \citep{metcalf_2007}. Widely-used ``absolute'' measures of fitness like $r$ and $K$ are derived directly from population ecology. By contrast, relative fitness is almost entirely divorced from population ecology. This discrepancy runs deeper than a simple difference in emphasis on absolute densities rather than relative frequencies. 

In uncrowded populations there is no discrepancy. Relative fitness simply represents  differences in $r$ ($r$-selection) \citep[pp. 26]{crow_1970}. The discrepancy emerges in crowded populations. Then the relative fitness description of selection assumes that $N$ is fixed, and fitness strictly involves the ability to increase in frequency at the expense of others (e.g. in the Wright-Fisher model). On the other hand, from quite general population ecological considerations, MacArthur famously argued that ``fitness is $K$'' in crowded populations ($K$-selection) \citep[pp. 149]{macarthur_1967}. To appreciate the import of MacArthur's argument, and see the nature of the discrepancy between these views of selection, we now briefly review MacArthur's argument. 
...
\end{equation}
where $f_1$ and $f_2$ are strictly decreasing functions of the population densities $n_1$ and $n_2$. Plotting the nullclines $f_1(n_1,n_2)=0$ and $f_2(n_1,n_2)=0$, it can be seen that one type will be excluded unless it is able to grow at higher densities than the other type in some region of $(n_1,n_2)$ space (Fig.~\ref{fig:Ksel}a). MacArthur defines four ``carrying capacities'' generally labeled $K$, as the intersection points of the nullclines with the $n_1$ and $n_2$ axes (i.e. $f_1(K_{11},0)=0$, $f_1(0,K_{12})=0$, $f_2(0,K_{22})=0$ and $f_2(K_{21},0)=0$). Assuming that the nullclines are linear, these $K$ values determine whether a region of higher-density growth exists for each type.

Note that only $K_{11}$ and $K_{22}$ are saturation densities analogous to ``$K$'' in the logistic model; $K_{12}$ and $K_{21}$ represent the effects of competition between types. For instance, under Lotka-Volterra competition we have $f_1(n_1,n_2)=r_1(1-\alpha_{11}n_1-\alpha_{12}n_2)n_1$. Then the nullclines are linear and $\alpha_{11}=1/K_{11}$ measures intra-type competitive effects, while $\alpha_{12}=1/K_{12}$ measures inter-type competitive effects.  Thus, for MacArthur ``fitness is $K$'' in the sense that selection in crowded populations either favors the ability to keep growing at ever higher densities, or the ability to stop the growth of competitors at lower densities. This general idea applies even if the nullclines are so curved that the ``$K$'' values themselves do not give much information about the regions of high-density growth.
...
Clearly MacArthur's claim is not compatible with the constant-$N$, relative fitness description of selection, in which $n_1$ and $n_2$ change along a line defined by $n_1+n_2=N$, and population density is otherwise irrelevant (Fig.~\ref{fig:Ksel}b). In this case, all four $K$ values are equal to $N$ [not!], and the $f_1$ and $f_2$ nullclines do not even exist. Since $N$ is fixed, it is not possible to persist at higher densities than $N$, nor is it possible to suppress the density of a competitor at densities lower than $N$
