...
We then explore some time-dependent behavior of our extended lottery model. Taking an example inspired by recent studies of rapid, seasonal evolution in \textit{Drosophila} \citep{bergland_14}, we discuss how environmental fluctuations might stabilize polymorphisms when population density is cyclical. 
...
Ideal environments are characterized by the near-absence of stress and disturbance, so that survival is easy ($d\ll 1$) and growth and reproduction can be rapid ($b\gg d$). The propagule density is determined by the magnitude of $b/d$, and hence $L\gg 1$ (Appendix C).
...
Not surprisingly, mutations which increase $c$ are effectively neutral when $L\ll 1$ and adaptive when $L\gg 1$. However, selection in our density-dependent lottery model does not favor improvement in $b$ versus $d$. The same is true for proportional $c$-mutations $c_j=(1+\epsilon) c_i$ when $L\gg 1$ in ideal and stressed (HD) environments.

In disturbed environments, where $\Delta_+ n_i$ is replaced by $(1-d_i)\Delta_+ n_i$, the argument for equal selection on $b$ and $d$ is more subtle and contingent. The bulk of the high mortality $d\approx 1$ is environmental and unavoidable. This implies that $\epsilon$ mutations in the remaining genetic part of mortality have a reduced impact on total $d$, suggesting the primacy of $b$ under disturbance. However, this effect is easily matched by the fact that $d_i\approx 1$ and that disturbance mortality affects both juvenile recruitment and adult mortality. In Appendix C, we argue that $b$ and $d$ mutations should contrive to have comparable effects on fitness in the disturbance case (assuming that $d$ can evolve at all) because the fitness benefit of improving $d$ declines as $d$ improves.
