...
\end{equation}
Since no population has unlimited resources, the functions $f_1$ and $f_2$ must decline to zero if either $n_1$ or $n_2$ are made sufficiently large. The outcome of selection can then be determined by the shape of the nullclines $f_1(n_1,n_2)=0$ and $f_2(n_1,n_2)=0$. Specifically, a type will be excluded if its nullcline is completely contained in the region bounded by the other type's nullcline. For a type to have the possibility of persisting, it must be able to keep growing to higher densities than the other type can tolerate in some region of $(n_1,n_2)$ space (Fig.~\ref{fig:Ksel}a). 

MacArthur's argument is clearly incompatible with a narrow interpretation of relative fitness models as selection along lines defined by $n_1+n_2=N$ for each $N$ (Fig.~\ref{fig:Ksel}b). In this case the $f_1$ and $f_2$ nullclines are not even defined. Moreover, at face value MacArthur's, conclusion that ``fitness is $K$'' in crowded populations \citep[pp. 149]{macarthur_1967} seems to be incompatible with constant $N$. In particular, the logistic model and its bi-allelic diploid variants are widely used to analyze MacArthur's ``$K$-selection'' in which case positive selection is necessarily tied to increases in $N$. More sophisticated models using the Lotka-Volterra competition equations, or explicit resource consumption, also intrinsically tie high-density selection to changes in population density. This raises the possibility that the ubiquitous constant-$N$, relative fitness description of selection could be incompatible with a huge class of population ecological processes occurring in nature and the lab.

Here we introduce a novel density-dependent model of population growth based on territorial contests, and show that when this model reaches a demographic steady-state, the constant-$N$, relative fitness picture emerges. We show that our model is entirely consistent with the claim that ``fitness is $K$'' in crowded populations when MacArthur's very liberal use of ``$K$'' is taken into account. In particular, MacArthur's argument does not justify the widespread intuition that selection in crowded environments  attempts to maximize population size is a consequence of 



argument is widely interpreted to mean that positive selection in crowded populations involves the ability to keep growing at ever higher population densities [citations]

Moreover, it is not compatible with most verbal interpretations of MacArthur's argument, 
...
\citep{anderson_1971}
 ...
 with the assumption of constant $N$, although as we will discuss below, it is a narrow interpretation of MacArthur's argument. 
