...
We can now determine which mutational changes in the traits $b, c$ and $d$ will be favored under different environmental conditions. Of particular interest are Grime's ``disturbance'', ``stress'' and ``ideal'' environmental archetypes. 

To proceed, we map these archetypes to different parameter regimes in our model. The ideal archetype is defined by low extrinsic mortality $d_i\ll 1$ and high birth rates, such that $b_i$ significantly exceeds $d_i$, and so  population density $N/K$ is high, and $L\gg 1$. In this case, territorial contests are important.

The disturbance archetype is defined by unavoidably high extrinsic mortality due to direct physical destruction of individuals, to the extent that population density is kept low $N/K\ll 1$. This means that, even though $b_i$ may be quite high, it cannot greatly exceed $d_i$, and that  propagule densities are low $l_i\ll 1$. The terms proportional to $c_i/\overline{c}$ in Eq. \eqref{eq:master} are then negligible, and $\Delta_+ n_i$ depends primarily on $b_i$.
