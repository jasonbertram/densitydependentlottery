...
We now outline the basic properties of selection on $b$, $c$ and $d$ in the variable-density lottery. 

Eq.~\eqref{eq:Nmaster} shows that $c$ is not a density-regulating trait: $c$ only affects the relative likelihood for each type to win a contested territory, not the number of territories that are contested in the first place. Thus, selection between types which only differ in $c$ occurs without causing $N$ to change. However, $c$-selection is density-dependent, with the strength of selection (measured by the difference between types in the per-capita growth rate $\Delta n_i/n_i$) peaking at an intermediate density (Fig.~\ref{fig:DDS_lottery}). This intermediate peak occurs because at low density most territories are claimed without contest, whereas at high density few unoccupied territories are available to be contested. 

The birth and mortality rates $b$ and $d$ do regulate density. To see how selection on $b$ and $d$ depends on density, we write Eq.~\eqref{eq:masterequalc} in the form
\begin{equation}
...
Equipped with an understanding of selection in the variable-density lottery model, we now turn to the broader issue of how density responds to selection \citep{prout_1980}. Going back to some of the earliest work on the topic \citep{kostitzin_1939,christiansen_2004}, most models of selection in density-regulated populations have some form of ``$K$-selection'' in the sense that selection in crowded populations increases saturation density. This makes sense intuitively because births balance deaths at equilibrium, and adaptive mutants must overcome the existing balance to expand. Yet, as we saw in the previous section, $c$-selection has no effect on population density in the variable-density lottery model. To make sense of how this property of $c$-selection fits with more familar population growth models, we revisit MacArthur's analysis of $K$-selection \citep{macarthur_1967}.
