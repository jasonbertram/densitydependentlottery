...
In the Introduction we described MacArthur's general conclusions about the behaviour of Eq.~\eqref{eq:macgeneral} based on the shape of the nullclines $f_1(n_1,n_2)=0$ and $f_2(n_1,n_2)=0$, but did not specify the role of ``$K$''. 

The symbol ``$K$'' enters the argument as the general label for the four intersection points of the nullclines with the $n_1$ and $n_2$ axes, specifically $f_1(K_{11},0)=0$, $f_1(0,K_{12})=0$, $f_2(0,K_{22})=0$ and $f_2(K_{21},0)=0$. These $K$ values determine whether a region of higher-density growth exists for each type, provided that the nullclines are close to being straight lines. 
