...
Here we argue that the simplified models of density-regulated growth mentioned above are misleading in important ways. These ultimately derive from their failure to account for ``reproductive excesses'', that is, an excess of juveniles that are more fragile than their adult counterparts \citep{turner1968population}. Reproductive excesses allow selection to be concentrated at a juvenile ``bottleneck'', potentially keeping the density of adults constant even if selection is strong. The reproductive excess concept featured prominently in early debates on the regulation of population density (e.g. \citealt{nicholson_1954}), and is also familiar to evolutionists, particularly in relation to Haldane's ``cost of selection'' \citep{haldane_1957,turner1968population}. Reproductive excesses are also implicit in foundational evolutionary-genetic models like the Wright-Fisher, where each non-overlapping generation involves the production of an infinite number of zygotes, a constant number $N$ of which are sampled to form the next generation of adults. Likewise, in the Moran model, a juvenile is available to replace a dead adult every iteration no matter how rapidly adults are dying, and as a result $N$ remains constant. 

Nevertheless, few studies of density-dependent selection have incorporated reproductive excesses. This requires density-regulated population growth in which the excess is finite and, in general, density-dependent. Such models are potentially much more complicated than those with no excess (e.g. logistic) or an infinite excess (e.g. Wright-Fisher). Nei's competitive selection model incorporated a finite reproductive excess to help clarify the ``cost of selection'' \citep{nei1971fertility,nagylaki_1992}, but used an unusual representation of competition based on pairwise interactions defined for at most two different types, and was also restricted to equal fertilities for each type. In more complex models with detailed age structure (as opposed to simply juveniles vs adults), it is usually assumed that the density of a ``critical age-group'' controls the population's response to crowding. The focus is then on the density in the critical age group,  and the role of density dependence is surprisingly similar to that found in models without any age structure \citep[Chap. 4]{charlesworth_1994}. For example, selection proceeds in the direction of increasing equilibrium density in the critical age-group, akin to classical $K$-selection \citep[pp. 148]{charlesworth_1994}. This gives little insight into the role of reproductive excesses in stabilizing adult densities. 
