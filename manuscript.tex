...
\caption{\label{fig:approx_details} Horse.}
\end{figure}

Starting with Eq.~\eqref{eq:meanfieldr}, we have $\langle x_j \rangle_{\tilde{p}}=l_j/C$, where $C=1-e^{-(L-l_i)}$, and the corresponding variance is given by
\begin{align}
...
\end{align}
Since $\sigma^2_{\tilde{p}}(\sum_{j\neq i} c_j x_j)=\sum_{j\neq i} c_j^2\sigma^2_{\tilde{p}}(x_j)$, and the first term in Eq. \eqref{eq:varr} is negative (because $C<1$), we obtain an upper bound on the relative fluctuations in $\sum_{j\neq i} c_j x_j$, 
\begin{equation}
\frac{\sigma(\sum_{j\neq i} c_j x_j)}{\langle\sum_{j\neq i} c_j x_j\rangle}=C^{1/2}\frac{\left(\sum_{j\neq i}c_j^2 ((1-\frac{1}{C})l_j^2+l_j)\right)^{1/2}}{\sum_{j\neq i}c_j l_j}<C^{1/2}\frac{\left(\sum_{j\neq i}c_j^2 l_j\right)^{1/2}}{\sum_{j\neq i}c_j l_j}. \label{eq:cvr}
\end{equation}

Suppose that the $c_j$ have similar magnitudes (their ratios are of order one). Then the right hand side of Eq.~\eqref{eq:cvr} is approximately equal to $\sqrt{\frac{1-e^{-(L-l_i)}}{L-l_i}}$. This quantity is small for both low and high nonfocal densities. The worst case scenario occurs when $L-l_i$ is of order one. Then the  Eq.~\eqref{eq:cvr} gives a relative error of approximately $50\%$

Our numerical results (Fig. \ref{fig:simcomp}) confirm that the relative errors are indeed small.
