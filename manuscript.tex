...
For haploids growing in discrete time steps, the change in the abundance $n_i$ of type $i$ over a time step can be expressed as $\Delta n_i=(W_i - 1)n_i$ where $W_i$ is absolute fitness. The corresponding change in frequency is $\Delta p_i=\left(\frac{W_i}{\overline{W}}-1\right) p_i$, where $\overline{W}=\sum_i W_i p_i$. In continuous time, the Malthusian parameter $r_i$ replaces $W_i$ and we have $\frac{d n_i}{dt}=r_in_i$ and $\frac{d p_i}{dt}=(r_i-\overline{r}) p_i$ \citep[pp. 26]{crow_1970}. In the particular case that two types are present, a mutant labeled $1$ and a wildtype labeled $2$ for instance, then the continuous time equation for the mutant takes the familiar form
\begin{equation}
\frac{d p_1}{dt}=s p_1(1-p_1), \label{eq:canonical}
\end{equation}
where $s=r_1-r_2$ is the selection coefficient. Note that in the discrete case we can replace the $W_i$ with any set of values proportional to the $W_i$ and the changes in frequency $\Delta p_i$ will be unaffected. These values, which give no information about the dynamics of total population density $N=\sum_i n_i$, are called ``relative fitnesses'' \citep[pp. 468]{barton_2007}. Similarly, the selection coefficient and frequency dynamics in Eq.~\eqref{eq:canonical} would be unaffected if we were to add an arbitrary constant to both $r_1$ and $r_2$ (these would be relative log fitnesses).  

In the absence of crowding, each type grows exponentially at a constant ``instrinsic'' rate $r_i$ (for concreteness, we will primarily use Eq.~\eqref{eq:canonical} as our point of comparison but similar logic applies in the discrete time case). The selection coefficient $s$ is then a constant equal to the difference in intrinsic population growth rates. The situation is more complicated when we allow for population crowding. Since crowded and uncrowded conditions can favor quite different traits \citep{travis_2013}, the Malthusian parameters $r_i$ may respond differently to changes in population density $N$, in which case the selection coefficient will be density-dependent. Eq.~\eqref{eq:canonical} would then need to supplemented with a model for the dynamics of $N$ to describe how the allele frequencies will change over time. The density and type frequency dynamics will in general be coupled \citep{travis_2013}. Thus, in a general crowded population the simple procedure of assigning constant relative fitness values to different types has to be replaced with an ecological description of absolute growth rates --- a substantially more complicated problem. Note that frequency-dependent selection does not raise such a severe problem, because we still only need to model the type frequencies, and not also the ecological variable $N$. 

In practice, many population genetics models assume that $N$ has reached some fixed equilibrium value, and each type is assigned a constant relative fitness. Selection is typically assumed to operate through viability (e.g. \citealt[pp. 61]{gillespie_2004}), but other important population ecological assumptions --- such as how density is regulated --- are left unspecified. Beyond being conceptually unsatisfactory \citep{doebeli_2017}, we might also doubt whether this density-independent-$s$, constant-$N$ setup is consistent with the ecology of many populations. Much of the seminal literature on density-dependent selection revolves around the connection between selection in crowded populations and the ability to keep growing at higher densities \citep{macarthur_1967,roughgarden_1979}. The classic example is the logistic model, where the type with the greatest equilibrium population density ``$K$'' excludes the others (Fig.~\ref{fig:Ksel}a). Similarly, the ``$R^*$ rule'', a central tenet of resource competition theory, states that when growth is limited by a single homogeneous consumable resource, the type able to deplete the resource to the lowest equilibrium density $R^*$ (which will often entail a different equilibrium population density) excludes the others \citep{grover_1997}. In these examples, both $N$ and $s$ change during, and as a result of, adaptive sweeps. The Lotka-Volterra competition model also couples selection in crowded populations to density except in special cases \citep{smouse_1976,mallet_2012}. It would therefore seem that the ubiquitous density-independent-$s$, constant-$N$ description of selection is incompatible with a huge class of population ecological processes driving selection (Fig.~\ref{fig:Ksel}b).
...
In light of this, and the related difficulties posed by frequency-dependent selection, relative fitness values that are independent of type frequencies and population density are typically justified as an approximation that holds when selection is weak (\citealt{kimura1969natural}; \citealt[pp. 277]{ewens_2004}; \citealt[Chap. 4]{charlesworth_1994}) and density is changing much slower than type frequencies \citep[Fig. 9.5]{otto_2011}. Under these conditions, the selection coefficient may depend on density and type frequencies, but since these change slowly the selection coefficient may remain approximately constant over several generations. In the case of density-dependence, this time interval of constancy could include entire selective sweeps. On the other hand, it is known in both simple models like the logistic \citep{kimura1969natural}, and in complex age-structured models \citep[Chap. 4]{charlesworth_1994}, that frequency trajectories under strong density- or frequency-dependent selection can differ substantially from the constant-$s$ approximation. 

Yet knowing that the constant-$s$ approximation can fail \textit{in general} when selection is strong gives only a rough picture of the domain and manner in which the constant-$s$ approximation breaks down. For instance, wild \textit{Drosophila} is an important model system in genetics, and exhibits strong seasonally-alternating selection in unison with large ``boom-bust'' density cycles \citep{messer_2016,bergland_14}. Are we compelled to switch to a more ecologically-detailed evolutionary model based on Malthusian parameters or birth/death rates in this system? And if we make this switch, how much ecological detail do we need? Perhaps the strongest justification of constant relative fitness models is that they are a powerful idealization. Thus, before abandoning them it is important to have a better understanding when and how we should do so when selection is not weak, and demography is not stable. 

The rough picture given by the weak-selection approximation is missing at least one key element: virtually all models of density-dependent selection do not have a ``reproductive excess'' of juveniles that are more fragile than their adult counterparts \citep{turner1968population,kimura1969natural,nei1971fertility}. Reproductive excesses allow selection to be concentrated at a juvenile ``bottleneck'', which means that the density of adults can remain constant even if strong selection is occurring on juveniles --- rather different from the classic $K$-selection scenario. Although similar ideas featured prominently in   early debates on the regulation of population density (coincidentally grounded in experiments on flies; \citealt{nicholson_1954}), much of the reproductive excess literature revolves around Haldane's ``cost of selection'' \citep{haldane_1957}, and is also poorly integrated with population ecology. For instance, \cite{kimura1969natural} simply took constant $N$ as a requirement and then derived variants of the logistic model consistent with this. \cite{nei1971fertility} proposed a model with an explicit representation of reproductive excess, but used an unusual model of competition based on pair-wise interactions which was only defined for at most two different types. As a result, the role of reproductive excesses in justifying the use of constant relative finesses is largely verbal.

Here we study the population ecology of relative fitness using a novel model of density-dependent population growth based on territorial contests. We restrict our attention to asexual haploids with little or no age structure, since it is then easier to evaluate how the success or failure of the constant-$s$ description is tied to the underlying population ecological assumptions. Our starting point is the classic lottery model which was developed by ecologists to study competition driven by territorial contests in reef fishes and plants \citep{sale_77,chesson_1981}. The classic lottery incorporates a reproductive excess, and fitness involves a product of fertility and juvenile viability akin to standard population genetic models of selection (e.g. \citealt[pp. 185]{crow_1970}). The classic lottery also assumes constant $N$, and generations can overlap. Our first task is to generalize the lottery model to create a variable-density lottery (sections ``Model'' and ``Analytical approximation of the variable-density lottery''). 

Equipped with our new variable-density lottery model, we outline its selection properties (section ``Density-dependent selection in the variable-density lottery''). Next, we discuss
selection on the ability to contest territories, which behaves like a pure constant-$N$, relative fitness trait, and discuss how this fits with the classical notion of $K$-selection  (section ''The response of density to selection; $c$-selection versus $K$-selection''). We then consider selection on density-regulating traits (section ``Density-regulating traits and the threat of strong selection''), and conclude by contrasting the classical density-dependent selection literature with our results (``Discussion'').
